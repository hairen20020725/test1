...
**现象2：超长括号说明（严重）**
表格中出现了超长的括号说明，包含产品选择逻辑解释。

**示例**：
```markdown
| 房间名称 | 房间面积 | 空调型号 | 数量 |
|---------|---------|---------|------|
| 次卧 | 12㎡ | 美的 KFR-26GW（知识库无此型号则用相近逻辑选1匹机，假设库存有可替代产品，此处按逻辑选split-001系列相近规格，实际需按库存调整，若严格按知识库则主次卧都用split-001，因知识库无更小规格明确信息，按面积1.5匹也适用12㎡且静音舒适） | 1台 |
```
...
**正确**：
```markdown
| 次卧 | 12㎡ | 美的 KFR-26GW | 1台 |

**方案说明**：
- 次卧12㎡选用1匹空调，根据知识库相近规格选择
```
...
| 模糊数值 | 约30㎡ | 30㎡ | 不能使用"约" |
| 短括号说明 | 格力 KFR-35GW（推荐） | 格力 KFR-35GW | 不能添加括号 |
| 超长括号说明 | 美的 KFR-26GW（知识库无此型号则用相近逻辑选1匹机...） | 美的 KFR-26GW | 绝对禁止超长说明 |
| 相对描述 | 根据客厅大小 | 15㎡ | 必须是绝对数值 |
| 建议词汇 | 建议1台 | 1台 | 只写数据 |
| 可选方案 | 1台或2台 | 1台 | 只写确定方案 |
| 范围描述 | 15-18㎡ | 18㎡ | 只写确定值 |
| 待定标记 | 待定 | 18㎡ | 必须确定 |
| 逻辑说明 | 若严格按知识库则... | （放表格外） | 逻辑说明必须在表格外 |
...
- 🎯 **逐列规范**：每列都有明确要求
- 📊 **示例对照**：正确和错误对比（包括超长说明案例）
- 🚫 **禁止列表**：明确不能使用的内容（包括逻辑说明）
- 💡 **原则说明**：解释为什么这样要求
- ✂️ **简短原则**：表格外说明也要简短
