...
export async function generateKnowledgeBaseFromDB(
  ...
  // 将历史案例放在最前面，强调其重要性
  knowledge += '## 一、历史成功案例（重点参考）\n\n';
  knowledge += '**说明**：以下是我们过往完成的真实案例，这些方案都经过实际验证，获得了客户的好评。在为新客户推荐方案时，请优先参考这些成功案例，特别是与当前户型相似的案例。\n\n';
  ...
  cases.forEach((case_, index) => {
    ...
    case_.solution.products.forEach((p: any) => {
      ...
      if (product) {
        knowledge += `- ${p.room}: ${product.brand} ${product.model} (产品ID: ${p.productId}) × ${p.quantity}台\n`;
        knowledge += `  - 适用面积: ${product.suitableArea.min}-${product.suitableArea.max}㎡\n`;
        knowledge += `  - 安装位置: ${p.installPosition}\n`;
      }
    });
  ...
  knowledge += '\n## 二、现有库存产品\n\n';
  knowledge += '**说明**：以下是我们当前的库存产品。在推荐方案时，请优先选择历史案例中已使用过的产品，如果案例产品缺货，再考虑同品牌同系列的替代产品。\n\n';
  ...
  knowledge += '\n## 三、推荐原则（必须遵守）\n\n';
  knowledge += '1. **历史案例优先**: 首先寻找与当前户型最相似的历史案例（面积、户型、朝向），优先采用其方案类型和产品配置\n';
  knowledge += '2. **产品选择优先级**:\n';
  knowledge += '   - 第一优先：历史案例中已验证成功的产品（有客户反馈）\n';
  knowledge += '   - 第二优先：与案例产品同品牌同系列的产品\n';
  knowledge += '   - 第三优先：知识库中有库存的其他产品\n';
  knowledge += '3. **方案类型参考**: 优先采用相似案例的方案类型，除非有充分理由调整\n';
  knowledge += '4. **经验传承**: 充分利用历史案例的经验总结和注意事项\n';
  knowledge += '5. **库存确认**: 优先推荐有库存的产品，确保方案可立即实施\n';
  knowledge += '6. **面积精准匹配**: 根据房间面积选择合适匹数，参考案例的配置经验\n';
  knowledge += '7. **预算参考**: 参考相似案例的预算范围，为客户提供合理的预算估算\n';
  knowledge += '8. **使用场景优化**: 卧室优先静音，客厅优先制冷量，老人房优先健康功能\n';
  knowledge += '9. **能效优先**: 优先推荐一级能效产品，长期使用更省电\n';
  knowledge += '10. **关注促销**: 在满足需求的前提下，优先推荐有促销活动的产品\n';
