...
**核心原则**：表格单元格必须严格按照表头要求填写具体数据，绝对禁止任何形式的说明、解释、备注。

**每列的填写规范**：
1. **房间名称**：只写房间名称（如"客厅"、"主卧"、"次卧1"），不得写"适配XX"、"根据XX"等
2. **房间面积**：只写具体数值+单位（如"30㎡"、"18㎡"），不得写"约XX"、"适配XX面积"等
3. **空调型号**：只写完整型号（如"格力 KFR-72LW"），必须是知识库中的真实产品，直接选择最合适的型号，不需要解释
...
- ❌ 绝对禁止：任何形式的括号"（）"及其内容
- ❌ 绝对禁止：任何解释性文字，无论长短
- ❌ 绝对禁止："适配主卧面积"、"根据客厅大小"、"约30㎡"
- ❌ 绝对禁止："可选"、"建议"、"推荐"、"假设"、"若"、"则"等词汇
- ❌ 绝对禁止：产品选择逻辑说明（如"知识库无此型号则..."）
- ❌ 绝对禁止：任何超过5个字的非数据内容

**产品选择原则**：
- 从知识库中直接选择最接近的产品
- 不需要在表格中解释为什么选择这个产品
- 所有解释必须放在表格外的"方案说明"部分

**正确示例**：
| 主卧 | 18㎡ | 格力 KFR-35GW | 1台 | 3500W | 1.5匹 | 950×320×215mm | ¥2,899 | ¥2,899 |
| 次卧 | 12㎡ | 美的 KFR-26GW | 1台 | 2600W | 1匹 | 848×289×204mm | ¥2,199 | ¥2,199 |

**错误示例（绝对不能出现）**：
| 主卧 | 适配主卧面积 | 格力 KFR-35GW | 1台 | 3500W | 1.5匹 | 950×320×215mm | ¥2,899 | ¥2,899 |
| 次卧 | 12㎡ | 美的 KFR-26GW（知识库无此型号则用相近逻辑选1匹机，假设库存有可替代产品） | 1台 | 2600W | 1匹 | 848×289×204mm | ¥2,199 | ¥2,199 |
| 客厅 | 约30㎡ | 格力 KFR-72LW（推荐） | 1台 | 7200W | 3匹 | 1850×700×320mm | ¥5,999 | ¥5,999 |

**方案说明**（表格外单独说明，简短为主）：
- 如需解释产品选择逻辑，在此处简短说明（1-2句话）
- 如有特殊配置考虑，在此处简短说明
- 如有可选方案，在此处简短说明
- 避免冗长解释，重点突出
